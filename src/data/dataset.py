...
class SamsumDataset(Dataset):
        ...
        if self.extra_context == True:
            if self.paracomet == False:
                ...
                if self.roberta:
                    print("ROBERTA ON!")
                    with open(
                        f"{comet_samsum_dialog_path}/roberta_nli/roberta_classified_top1_{self.split_type}.json"
                    ) as f:
                ...
                if self.roberta:
                    print("ROBERTA ON!")
                    with open(
                        f"{paracomet_samsum_dialog_path}/roberta_nli/paracomet_samsum_roberta_classified_top1_{self.split_type}.json"
                    ) as f:
                        self.roberta_classified_z = json.load(f)
                if self.sentence_transformer:
                    with open(
                        f"{paracomet_samsum_dialog_path}/sentence_transformer/paracomet_{self.split_type}_z.json"
                    ) as f:
                        self.sentence_transformer_classified_z = json.load(f)
    ...
    def __getitem__(self, index):
        if self.extra_context == False:
            ...
            encoded_dialogue = self.tokenizer(
                dialogue, padding="max_length", truncation=True, max_length=self.encoder_max_len, return_tensors="pt"
            )
    ...
    def __init__(
        ...
        if self.extra_context == True:
            if self.paracomet == False:
                ...
                ###########################
                if self.idiom:
                    with open(f"../data/COMET_data/comet/dialogue/dialogsum/idioms/preprocessed_{self.split_type}_data.json") as f:
                        self.dialogue_comet_inference = json.load(f)
                else:
                ...
                if self.idiom:
                    with open(f"src/data/COMET_data/paracomet/dialogue/dialogsum/idioms/preprocessed_{self.split_type}_data.json") as f:
                        self.dialogue_comet_inference = json.load(f)
                else:
                    with open(
                ...
                if self.roberta:
                    with open(
                        f"src/data/COMET_data/paracomet/dialogue/dialogsum/roberta_nli/paracomet_dialogsum_roberta_classified_top1_{self.split_type}.json"
                    ) as f:
                        self.roberta_classified_z = json.load(f)

                if self.sentence_transformer:
                    with open(
                        f"src/data/COMET_data/paracomet/dialogue/dialogsum/sentence_transformer/paracomet_{self.split_type}_z.json",
                        "r",
                    ) as f:
                        self.sentence_transformer_classified_z = json.load(f)
        ...
        if self.extra_supervision == True:
            if self.split_type == "train":
                if self.paracomet == False:
                    ...
                    # CODE FOR COMET
                    ######################
                    with open(f"src/data/COMET_data/comet/summary/dialogsum/comet_train_w.json") as f:
                        self.summary_comet_inference = json.load(f)

                    if self.roberta:
                        with open(
                            f"src/data/COMET_data/comet/dialogue/dialogsum/roberta_nli/roberta_classified_top1_w.json"
                        ) as f:
                            self.roberta_classified_w = json.load(f)

                    if sentence_transformer:
                        with open(
                            f"src/data/COMET_data/comet/summary/dialogsum/sentence_transformer/comet_train_w.json", "r"
                        ) as f:
                            self.sentence_transformer_classified_w = json.load(f)
                    ...
                    # CODE FOR PARACOMET
                    ########################
                    with open("src/data/COMET_data/paracomet/summary/dialogsum/summary_train_split5_collated.json") as f:
                        self.summary_comet_inference = json.load(f)

                    if self.roberta:
                        with open(
                            "src/data/COMET_data/paracomet/summary/dialogsum/roberta_nli/roberta_classified_top1_w.json"
                        ) as f:
                            self.roberta_classified_w = json.load(f)

                    if sentence_transformer:
                        with open(
                            "src/data/COMET_data/paracomet/summary/dialogsum/sentence_transformer/paracomet_train_w.json",
                            "r",
                        ) as f:
                            self.sentence_transformer_classified_w = json.load(f)
                        ...
                        try:
                            while True:
                                if self.dialogue_comet_inference["train_" + self.id[index]][idx]["sentence"] not in (
                                    ...
                                    commonsense = self.dialogue_comet_inference["train_" + self.id[index]][idx][
                                        self.relation
                                    ][0].strip()
                                    # commonsense = commonsense.replace("PersonX","Person").replace("PersonY","Person")
                                    break
                        ...
                        try:
                            while True:
                                if self.dialogue_comet_inference["dev_" + self.id[index]][idx]["sentence"] not in (
                                    ...
                                    commonsense = self.dialogue_comet_inference["dev_" + self.id[index]][idx][
                                        self.relation
                                    ][0].strip()
                                    commonsense = commonsense.replace("PersonX", "Person").replace("PersonY", "Person")
                                    break
                        ...
                        try:
                            while True:
                                if self.dialogue_comet_inference["test_" + self.id[index]][idx]["sentence"] not in (
                                    ...
                                    commonsense = self.dialogue_comet_inference["test_" + self.id[index]][idx][
                                        self.relation
                                    ][0].strip()
                                    # commonsense = commonsense.replace("PersonX","Person").replace("PersonY","Person")
                                    break
        ...
        if self.extra_context == True:
            if self.paracomet == False:
                ...
                if self.roberta:
                    with open(
                        os.path.join(
                            DATA_DIR, f"RobertaClassifier/samsum/roberta_classified_top1_{self.split_type}.json"
                        )
                    ) as f:
                        self.roberta_classified_z = json.load(f)
        ...
        if self.extra_supervision == True:  # use commonsense w
            if self.split_type == "train":
                if self.paracomet == False:  # plain COMET
                    ...
                    if self.roberta:
                        with open(
                            os.path.join(DATA_DIR, f"RobertaClassifier/samsum/roberta_classified_top1_w.json")
                        ) as f:
                            self.roberta_classified_w = json.load(f)
                else:
                    with open(
