...
from src.config.enums import DatasetOptions, ExperimentPhase, FrameworkOption, ModelCheckpointOptions
from src.experiments.experiment import FewShotLearning
...
def load_checkpoint(args: Namespace):
    if args.framwork == FrameworkOption.BASIC_SICK:
...
def load_pretrained_model(args: Namespace, tokenizer, device):
    if args.framwork == FrameworkOption.BASIC_SICK:
        ...
        return AutoModelForCausalLM.from_pretrained(
            args.model_name, use_auth_token=args.hugging_face_token, torch_dtype=torch.float16, device_map=device
        )
    else:
...
def get_tokenizer(args: Namespace):
    ...
    if args.framwork == FrameworkOption.FEW_SHOT:
...
def get_finetune_args(args: Namespace) -> Seq2SeqTrainingArguments:
    ...
    elif args.framwork == FrameworkOption.BASIC_SICK_PLUS_PLUS:
        return Seq2SeqTrainingArguments(
            output_dir=args.finetune_weight_path,
            overwrite_output_dir=True,
            do_train=True,
