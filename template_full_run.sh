...
# Step 3: 
# login inside wandb with api key if needed for logging (be carefull to add adjust program params accordingly with this) 
...
# Note: if needed run python3 src/run.py --help to recive guide on the parameters available
...
PARAMS=(
    ...
    --framework <FRAMEWORK> # (see enums.py/FrameworkOption)
    --exp_name <ADD_EXP_NAME> # it will used for the run in wandb and as part of the name for the local logger (after data)
    --seed 516
    --phase <PHASE> # (see enums.py/ExperimentPhase - for training + test use "full")
    --dataset_name <DATASET_NAME> # (see enums.py/DatasetOptions)
    --model_name <MODEL_NAME> # (see enums.py/ModelCheckpointOptions)
